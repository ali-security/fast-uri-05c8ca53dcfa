...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // The reference test uses the "mailto" handler, which this line only gained
  // in v4. Use "ws" instead: its handler is the one that populates
  // resourceName/secure, so those fields are only set when the percent-encoded
  // scheme was decoded *before* the scheme handler was looked up.
  const ws = fastURI.parse('w%73://example.com/chat')
  t.equal(ws.error, undefined, 'ws parsing runs after decoding the scheme')
  t.equal(ws.scheme, 'ws', 'decoded ws scheme')
  t.equal(ws.resourceName, '/chat', 'ws handler populated resourceName')
  t.equal(ws.secure, false, 'ws handler populated secure')
  t.end()
})
