...
const { normalizeIPv6, removeDotSegments, recomposeAuthority, normalizeComponentEncoding, reescapeHostDelimiters, isIPv4, isIPLiteral, nonSimpleDomain } = require('./lib/utils')
const { SCHEMES, getSchemeHandler } = require('./lib/schemes')

// RFC 3986 scheme = ALPHA *( ALPHA / DIGIT / "+" / "-" / "." ). Deliberately
// restricted to ASCII letters: Unicode look-alikes such as U+212A KELVIN SIGN
// or U+017F LATIN SMALL LETTER LONG S case-fold onto ASCII letters and would
// otherwise let a non-scheme masquerade as a known one.
...
function parseWithStatus (uri, opts) {
  ...
  if (matches) {
    ...
    // Decode and validate the scheme up front. Deferring it until after the
    // authority has been resolved and the scheme handler picked lets a
    // percent-encoded scheme decode into a *different* scheme ("w%73" -> "ws"),
    // into an authority ("%2f%2f" -> "//"), or into control characters
    // ("%0d%0a" -> CRLF) once the value is handed back to the caller.
    ...
    if (parsed.host) {
      ...
      if (ipv4result === false) {
        // An IP literal is only well formed when the whole host is bracketed.
        // A host carrying a stray or unterminated bracket is never a valid
        // reg-name either, so reject it instead of letting it reach the IDN
        // canonicalization, where a lenient URL parser could repair it into a
        // different (attacker-chosen) host.
    ...
    if (!options.unicodeSupport && (!schemeHandler || !schemeHandler.unicodeSupport)) {
      ...
      if (parsed.host && !malformedIPLiteral && (options.domainHost || (schemeHandler && schemeHandler.domainHost)) && isIP === false && nonSimpleDomain(parsed.host)) {
...
function normalizeComparableURI (uri, opts) {
  ...
  if (typeof uri === 'object') {
    // Fail closed: serialize() throws on a component whose scheme decodes to
    // something that is not a valid RFC 3986 scheme, so equality must not
    // succeed for such a component.
    try {
